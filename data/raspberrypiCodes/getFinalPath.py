...
# === Firebase Realtime Listener for SearchedLocation ===
def on_snapshot(doc_snapshot, changes, read_time):
    ...
    for doc in doc_snapshot:
        ...
        if "SearchedLocation" in data:
            destination_coords = {
                "latitude": data["SearchedLocation"].get("latitude", 0.0),
                "longitude": data["SearchedLocation"].get("longitude", 0.0)
            }
            print(f"New Destination: {destination_coords}")

doc_ref = db.collection("helmet_locations").document("A1M2C4N88")
...
while True:
    try:
        doc = db.collection("helmet_locations").document("A1M2C4N88").get()
        data = doc.to_dict()
        if "CurrentLocation" in data:
            current_coords = {
                "latitude": data["CurrentLocation"].get("latitude", 0.0),
                "longitude": data["CurrentLocation"].get("longitude", 0.0)
            }
